...
git config --global alias.log-watch 'll'
git config --global alias.sl '!myFn() { while :; do clear; git --no-pager status; sleep 1; done }; myFn'
git config --global alias.status-watch 'sl'
